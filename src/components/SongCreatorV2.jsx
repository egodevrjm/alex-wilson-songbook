...
export default function SongCreator({ onSongCreated, onClose }) {
  ...
  const createManualSong = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (error.name === 'QuotaExceededError') {
        alert('Storage quota exceeded. The app has automatically migrated to a more efficient storage system. Please try again.');
      } else {
    ...
    <form onSubmit={createManualSong} className="space-y-6">
      ...
      {error && (
        <div className="p-3 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded-md">
          <p className="font-medium">Storage Notice</p>
          <p className="text-sm mt-1">The app is using an improved storage system to handle larger songbooks. Your data is safe.</p>
        </div>
      )}
